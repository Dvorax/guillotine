...
class Card(object):
    ...
    def activate(self, game):
        # stack is used to pass temporary objects between events
        stack = []

        try:
            for event, parameters in self.events:
                event(game, stack, **parameters)
        except events.NoUseException:
            print(' ------------------ Raised no use exception')
...
class NobleCard(Card):
    ...
    categories = {'Church': 'blue', 'Military': 'red', 'Royal': 'purple', 'Negative': 'gray', 'Civic': 'green'}

    def __init__(self, name, value, category, description=None, event_order=None, trigger='collection'):
...
action_cards = [
        ActionCard('Stumble', 'Move a noble forward exactly 1 place in line.', [
                (events.choose_from_line, {'from_front': 1}),
                (events.move, {'distance': 1})
        ]), ActionCard('Pushed', 'Move a noble forward exactly 2 places in line.', [
                (events.choose_from_line, {'from_front': 2}),
                (events.move, {'distance': 2})
        ]), ActionCard('Friend of the Queen', 'Move a noble backward up to 2 places in line.', [
                (events.choose_from_line, {'from_back': 1}),
                (events.choose_movement, {'distance': -2}),
                (events.move, {})
        ]), ActionCard('Ignoble Noble', 'Move a noble forward exactly 4 places in line.', [
                (events.choose_from_line, {'from_front': 4}),
                (events.move, {'distance': 4})
        ]), ActionCard("L'Idiot", 'Move a noble forward up to 2 places in line.', [
                (events.choose_from_line, {'from_front': 1}),
                (events.choose_movement, {'distance': 2}),
                (events.move, {})
        ]), ActionCard('Fainting Spell', 'Move a noble backward up to 3 places in line.', [
                (events.choose_from_line, {'from_front': 1}),
                (events.choose_movement, {'distance': -3}),
                (events.move, {})
        ]), ActionCard('Was That My Name?', 'Move a noble forward up to 3 places in line.', [
                (events.choose_from_line, {'from_front': 1}),
                (events.choose_movement, {'distance': 3}),
                (events.move, {})
        ])
