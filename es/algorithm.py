...
class EvolutionStrategy(object):
    ...
    def evolve(self, population, show_progress=True):
        ...
        for generation in range(self.termination_count):
            ...
            population = self.select_survivors(population, children)
            
            diff_sum = best_score = 0
            ...
            for individual in population:
                ...
                if score > best_score:
                    ...
                    best_score = score
                diff_sum += individual.square_difference()

            # if max(population) > best_individual:
            #     best_individual = max(population)

            if show_progress and generation % 1 == 0:
                # print('.', end='')
                print('Generation {}, Score: {}, Diff^2: {}, avg: {}'.format(
                        generation, best_individual.evaluate(), 
                        best_individual.square_difference(), 1.0 * diff_sum / len(population)))
                sys.stdout.flush()
    ...
    def select_survivors(self, parents, children):
        ...
        return sorted(parents + children)[-self.num_parents:]
